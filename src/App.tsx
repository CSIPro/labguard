        ...
        {LabValue.map((Labs) => (
          <li key={Labs.id}>
            ...
            <Link to={{ pathname: `/ListadoReporte/${Labs.Nombre}/${Labs.id}` }}>
              <button className="border-1 border-white px-4 py-2 rounded-lg bg-colorButtonOrange hover:bg-gray-300 text-black transition duration-300">Historial</button>
            </Link>
            <Link to={{ pathname: `/Reporte/${Labs.Nombre}/${Labs.id}` }}>
              <button className="border-2 border-white px-4 py-2 rounded-lg bg-colorButtonOrange hover:bg-gray-300 text-black transition duration-300">Hacer un reporte</button>
            </Link>
          </li>
        ))}
      </ul>
